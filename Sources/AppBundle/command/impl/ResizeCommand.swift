...
struct ResizeCommand: Command { // todo cover with tests
    ...
    func run(_ env: CmdEnv, _ io: CmdIo) -> Bool {
        ...
        let candidates = target.windowOrNil?.parentsWithSelf
            .filter { ($0.parent as? TilingContainer)?.layout == .dwindle }
            ?? []
        ...
        guard let parent else { return io.err("resize command doesn't support floating windows yet https://github.com/BarutSRB/HyprSpace/issues/9") }
