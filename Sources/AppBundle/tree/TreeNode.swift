...
class TreeNode: Equatable, AeroAny {
    ...
    var isBound: Bool { parent != nil } // todo drop, once https://github.com/BarutSRB/HyprSpace/issues/1215 is fixed
        ...
        switch getChildParentRelation(child: self, parent: parent) {
            case .tiling(let parent):
                ...
                if parent.layout != .dwindle {
                    die("Weight can be changed only for nodes whose parent has 'dwindle' layout")
                }
    ...
    func bind(to newParent: NonLeafTreeNodeObject, adaptiveWeight: CGFloat, index: Int) -> BindingData? {
        ...
        // 2. Misbehaved apps that abuse real window as popups https://github.com/BarutSRB/HyprSpace/issues/106 (the
