...
private func resizeWithMouse(_ window: Window) async throws { // todo cover with tests
    ...
    switch parent.cases {
        ...
        case .tilingContainer:
            ...
            let (lParent, lOwnIndex) = window.closestParent(hasChildrenInDirection: .left, withLayout: .dwindle) ?? (nil, nil)
            let (dParent, dOwnIndex) = window.closestParent(hasChildrenInDirection: .down, withLayout: .dwindle) ?? (nil, nil)
            let (uParent, uOwnIndex) = window.closestParent(hasChildrenInDirection: .up, withLayout: .dwindle) ?? (nil, nil)
            let (rParent, rOwnIndex) = window.closestParent(hasChildrenInDirection: .right, withLayout: .dwindle) ?? (nil, nil)
                ...
                if let parent, let startIndex, let pastTheEndIndex, pastTheEndIndex - startIndex > 0 && abs(diff) > 5 { // 5 pixels should be enough to fight with accumulated floating precision error
                    ...
                    window.parentsWithSelf.lazy
                        ...
                        .filter {
                            ...
                            return parent?.orientation == orientation && parent?.layout == .dwindle
                        }
